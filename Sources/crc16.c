/**
  ...
  * @file           : crc16.c
  * @version        : v1.0
  * @brief          : библиотека для подсчета crc16 табличным образом как в ModBus и др.
...
static const uint16_t _crc16tab[256]= 
{
...
// Reverse bit_order in single byte
...
/**
  * @brief  подсчет контрольной суммы CRC-8 ECSS-E-ST-50-52C (RMAP) для данных:  порядок данных, поступающих на вход регистра и перед финальным суммирование по модулю два, инвертируются
...
/**
  * @brief  подсчет контрольной суммы CRC-16 для кадров (как в AX25)
