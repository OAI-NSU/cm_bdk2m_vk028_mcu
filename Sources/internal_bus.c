...
void ib_init(typeIBStruct* ib_ptr)
{
	...
	ib_ptr->uart_inst = UART1;
	ib_ptr->uart.init = UART_Init;
	ib_ptr->uart.send_packet = UART_TxPacket;
...
int8_t ib_process_tp(void* ctrl_struct, uint64_t time_us, typeProcessInterfaceStruct* interface)
{
	...
	if ((time_us - ib_ptr->last_call_time_us) > (IB_PROCESS_PERIOD*1000)) {
		...
		//! транзакция, которая ожидает команд по ВШ для управления ЦМ из вне (например из КПА)
...
int8_t ib_run_transaction(typeIBStruct* ib_ptr, uint8_t dev_id, uint8_t f_code, uint16_t reg_addr, uint16_t reg_cnt, uint16_t* data)
{
	...
	if (f_code == MB_F_CODE_IDLE){
		...
		//
		ib_ptr->uart.send_packet(ib_ptr->uart_inst, ib_ptr->tx_data, ib_ptr->tx_len);
	}
	else{
		//
	}
	...
	if ((ib_ptr->transaction_type == MB_TRANSACTION_WRITE) || (ib_ptr->transaction_type == MB_TRANSACTION_READ)){
		while((timeout <= IB_UART_TIMEOUT_MS) || (ib_ptr->uart.packet_in_waiting(ib_ptr->uart_inst))){
			timeout += 1;
			Timer_Delay(1, 1); 
			if(ib_ptr->uart.packet_ready(ib_ptr->uart_inst))
			{
				break;
			}
		}
		...
		// //
		// mko_irq_ena(); // отключение прерываний, которые мешают пакетному приему
		// //
	}
	...
	memset(ib_ptr->rx_data, 0x00, sizeof(ib_ptr->rx_data));
	if (ib_ptr->uart.get_packet(ib_ptr->uart_inst, ib_ptr->rx_data, (uint16_t*)&ib_ptr->rx_len)) {
		//
		memset((uint8_t*)&ib_ptr->rx_frame, 0x00, sizeof(typeModBusFrameStruct));
		//
...
/**
  ...
  */
void mb_frame_form_packet(typeModBusFrameStruct* frame_ptr, uint8_t* data, uint16_t* leng)
{
	uint8_t l = 0, i = 0;
